...
def clear_all_tenders():
    ...
    with app.app_context():
        ...
        confirm = input(f"⚠️  This will delete {count} tender(s). Continue? (yes/no): ")
        
        if confirm.lower() in ['yes', 'y']:
            TenderResult.query.delete()
            db.session.commit()
            print(f"🗑️  Deleted {count} tender(s) from database")
            print("✅ Database cleared! Run a new scan to get results with strict keyword filtering.")
        else:
            print("❌ Cancelled")
