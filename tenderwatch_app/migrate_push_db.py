...
with app.app_context():
    ...
    print("🔄 Checking for push_subscription table...")
    
    if 'push_subscription' not in inspector.get_table_names():
        print("📦 Creating push_subscription table...")
        ...
        db.session.commit()
        print("✅ Push subscription table created successfully!")
    else:
        print("✅ Push subscription table already exists")
    ...
    if 'min_score_to_notify' not in settings_columns:
        print("📦 Adding min_score_to_notify column to app_settings...")
        db.session.execute(text(
            "ALTER TABLE app_settings ADD COLUMN min_score_to_notify FLOAT DEFAULT 50.0"
        ))
        db.session.commit()
        print("✅ min_score_to_notify column added!")
    else:
        print("✅ min_score_to_notify column already exists")
